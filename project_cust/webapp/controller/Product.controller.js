        ...
        return Controller.extend("projectcust.controller.Product", {
            ...
            _onPatternMatched: function(oEvent) {
                ...
                if(oArgu.mCode) {
                    ...
                    sMatcode = oArgu.mCode;
                    if(oArgu.mCode === 'CARE01' || oArgu.mCode === 'CARE02' || oArgu.mCode === 'CARE03' || oArgu.mCode === 'CARE04') {
                        sOrdstate = 'C';
                        this.byId("togglebtn").setVisible(false);
                    } else {
                        sOrdstate = 'P';
                        this.byId("togglebtn").setVisible(true);
                    }
                    // console.log(aFilter);
                }
                ...
                oBinding.attachEventOnce("dataReceived", function() {
                    ...
                    if (aItems.length > 0) {
                        ...
                        var mname = oContext.getProperty("Matname");

                        this.byId("page").setTitle(mname + '의 상품상세');

                        //자재명
                        this.byId("HeaderMatName").setTitle(mname);
                        sMatname = mname;
                        // 판매가
                        ...
                        sPrc1 = parseInt(sMatprice * 100, 10);                       
                        ...
                        //이미지를 위한 Matcode
                        var mcode = oContext.getProperty("Matcode");
                    
                        // 이미지 설정
                        var sImageUrl = this.setImageUrl(mcode);
            ...
            onAddToCart: function(oEvent){
                ...
                var sFinPrc = '';   //Saprice로 전달
                
                switch(sOrdstate) {
                    case "P" :
                        console.log('즉구 : ', sMatcode, sOrdstate, sCustcode, sMatname, sPrc1);
                        sFinPrc = sPrc1;
                        break;
                    case "R5" :
                        console.log('렌5 : ', sMatcode, sOrdstate, sCustcode, sMatname, sPrc2);
                        sFinPrc = sPrc2;
                        break;
                    case "R7":
                        console.log('렌7 : ', sMatcode, sOrdstate, sCustcode, sMatname, sPrc3);
                        sFinPrc = sPrc3;
                        break;
                    case "C":
                        console.log('케어 : ', sMatcode, sOrdstate, sCustcode, sMatname, sPrc1);
                        sFinPrc = sPrc1;
                        break;
                }
                ...
                var sData = {
                    ...
                    Quant : "1",
                    Unit : "EA",
                    Saprice : String(sFinPrc),
                    Currkey : "KRW"
                };
                console.log('장바구니에 담을 상품 : ',sData);
                

                var oDataModel = this.getOwnerComponent().getModel();
                oDataModel.setUseBatch(false);
                oDataModel.create("/CartEntitySet", sData, {
                    method: "POST",
                    success: function(data) {
                        sap.m.MessageToast.show("상품이 장바구니에 담겼습니다.");   
                        location.reload();
                    },
                    error: function(oError) {
                        console.log('장바구니에 상품 추가 중 오류 발생 : ', oError);
                    }
                })
